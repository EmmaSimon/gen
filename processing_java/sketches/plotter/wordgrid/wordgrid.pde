...
final String PROJECT_NAME = "wordgrid";
...
void draw() {
  ...
  drawState();
  translate(400, 200);
  String[] words = new String[] {"abiogenesis", "vortices", "protozoan", "telotroch", "hexaploid", "archaeon", "phosphorylation", "plastid", "autotrophic", "chloroplasts", "rhodophyceae", "photoheterotroph", "ectothiorhodospiraceae"};

  for (int c = 0; c < state.getInt("counter"); c++) {
    hf.textSize(round(noise(c+ 100) * state.getInt("multiplier")));
    String word = words[max(0, min(round(map(noise(c+200), 0, 1, -1, words.length + 1)), words.length - 1))];
    hf.text(word, round(negged(noise(c*100)) * 10 * state.getInt("multiplier")), round(c * 2 * state.getInt("multiplier") * noise(c*c)));
